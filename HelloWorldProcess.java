    ...
    public RunResult run() {
        if(stateVariable==0){
            ...
            if(lockres){ // locking successful
                ...
                int id1=OS.getInstance().Open("random 100"); // opening a random device
                int res=OS.getInstance().Write(id1,temp);
                OS.getInstance().Close(id1); // closing the device
            ...
            OS.getInstance().unlockMutex=OS.getInstance().currentProcess;
            OS.getInstance().mutex.Unlock(mutexId);
            OS.getInstance().freeMutex=OS.getInstance().currentProcess;
            OS.getInstance().mutex.ReleaseMutex(mutexId);
            OS.getInstance().currentProcess.mutexList.remove(mutexId);
